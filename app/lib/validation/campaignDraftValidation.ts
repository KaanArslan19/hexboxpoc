...
export const campaignDraftValidationSchema = Yup.object().shape({
  title: Yup.string().trim().max(60, 'Title must be 60 characters or less').transform((value) => value === '' ? null : value).nullable(),
  description: Yup.string().max(10000, 'Description must be 10000 characters or less').transform((value) => value === '' ? null : value).nullable(),
  email: Yup.string().transform((value) => value === '' ? null : value).nullable().test('email-format', 'Invalid email format', function(value) {
    if (!value) return true; // Allow empty/null
    return Yup.string().email().isValidSync(value) && value.length <= 60;
  }),
  phoneNumber: Yup.string().transform((value) => value === '' ? null : value).nullable().test('phone-format', 'Please enter a valid phone number', function(value) {
    if (!value) return true; // Allow empty/null
    return /^\+?[0-9]{1,4}[0-9\s.-]{5,}$/.test(value);
  }),
  fundAmount: Yup.number().transform((value) => (isNaN(value) || value === '' || value === null) ? null : value).nullable().test('fund-range', 'Fund amount must be positive and less than 1 trillion', function(value) {
  ...
  deadline: Yup.string().transform((value) => value === '' ? null : value).nullable(),
  location: Yup.string().max(60, 'Location must be 60 characters or less').transform((value) => value === '' ? null : value).nullable(),
  wallet_address: Yup.string().transform((value) => value === '' ? null : value).nullable().test('wallet-format', 'Wallet address must be a valid EVM address', function(value) {
    if (!value) return true; // Allow empty/null
    return /^0x[a-fA-F0-9]{40}$/.test(value);
  }),
  one_liner: Yup.string().max(80, 'One Liner must be 80 characters or less').transform((value) => value === '' ? null : value).nullable(),
  telegram: Yup.string().transform((value) => value === '' ? null : value).nullable().test('telegram-url', 'Telegram must be a valid URL format', function(value) {
    if (!value) return true; // Allow empty/null
    return Yup.string().url().max(100).isValidSync(value);
  }),
  discord: Yup.string().transform((value) => value === '' ? null : value).nullable().test('discord-url', 'Discord must be a valid URL format', function(value) {
    if (!value) return true; // Allow empty/null
    return Yup.string().url().max(100).isValidSync(value);
  }),
  website: Yup.string().transform((value) => value === '' ? null : value).nullable().test('website-url', 'Website must be a valid URL format', function(value) {
    if (!value) return true; // Allow empty/null
    return Yup.string().url().max(100).isValidSync(value);
  }),
  linkedIn: Yup.string().transform((value) => value === '' ? null : value).nullable().test('linkedin-url', 'LinkedIn must be a valid URL format', function(value) {
    if (!value) return true; // Allow empty/null
    return Yup.string().url().max(100).isValidSync(value);
  }),
  funding_type: Yup.string().transform((value) => value === '' ? null : value).nullable().test('funding-type-valid', 'Invalid funding type', function(value) {
    if (!value) return true; // Allow empty/null
