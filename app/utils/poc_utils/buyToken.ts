...
export const buyToken = async (user: string, token_address: string, amount: number) => {
    try {
        ...
        const holderExists = tokenDetails.holders.some((holder: any) => holder.address === user);
        ...
        const updatedToken = await client.db("hexbox_poc").collection("tokens").updateOne(
