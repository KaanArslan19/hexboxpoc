...
export const POST = async (req: NextRequest) => {
    try {
        ...
        const userWalletAddress = session.user?.name;
        ...
        const events = receipt.logs
            .filter(log => log.address.toLowerCase() === CONTRACTS.USDCFundraiserFactory.fuji.toLowerCase())
            .map(log => {
                try {
        ...
        const fundraiser = new ethers.Contract(
            ...
            initializeChainlink(params: string, overrides?: { gasLimit: number }): Promise<any>;
        ...
        try {
            ...
            const fundraiserLinkBalance = await linkToken.balanceOf(fundraiserContractAddress);
            ...
            if (fundraiserLinkBalance < ethers.parseEther("1")) {
                ...
                const contractLinkBalance = await linkToken.balanceOf(fundraiserContractAddress);
