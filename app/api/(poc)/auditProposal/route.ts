...
export const POST = async (req: NextRequest, res: NextResponse) => {
    try {
        ...
        const auditProposalResponse = await auditProposal(session?.user?.name as string, proposal_id as string, approveBoolean)
        console.log(auditProposalResponse)

        if (typeof auditProposalResponse === 'object' && auditProposalResponse !== null && 'error' in auditProposalResponse) {
            return NextResponse.json({ error: auditProposalResponse.error }, { status: 400 });
        }
